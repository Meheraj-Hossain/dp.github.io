...
let swipeStatus = function(event, phase, direction, distance, duration, fingerCount) {
...
let swipeLeft  = function(event, direction, distance, duration, fingerCount) {
...
let swipeRight = function(event, direction, distance, duration, fingerCount) {
  //This only fires when the user swipes left
  var goTo = $('nav li.cur').index();
  goTo--;
  swipegoryUpdate(goTo);
    ...
    listItems.each(function(i) {
      ...
      if(i == prev) {
        ...
        if(autoAdjustHeight == true) {
          $('.swipegory').css('height', pane.height() + li.height() + li.height());
        }
